package com.example.cloudalibaba.sentinel.circuitbreak;
...
public class SlowRatioCircuitBreakerDemo {
            ...
            Thread entryThread = new Thread(() -> {
                while (true) {
                    ...
                    try {
                        ...
                        // RT: [40ms, 60ms)
                        sleep(ThreadLocalRandom.current().nextInt(45, 60));
        ...
        EventObserverRegistry.getInstance().addStateChangeObserver("logging",
                (prevState, newState, rule, snapshotValue) -> {
                    ...
                    if (newState == CircuitBreaker.State.HALF_OPEN) {
                        System.err.println(String.format("%s -> HALF_OPEN at %d, snapshotValue=%.2f", prevState.name(),
                                TimeUtil.currentTimeMillis(), snapshotValue));
                    } else {
    ...
    private static void initDegradeRule() {
        ...
        DegradeRule rule = new DegradeRule(KEY)
                ...
                .setCount(50)
                // Retry timeout (in second)  熔断时长 单位s
                .setTimeWindow(10)
